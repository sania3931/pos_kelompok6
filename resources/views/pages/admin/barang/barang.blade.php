...
@section('css')
  <link rel="stylesheet" href="{{ asset('css/manage_barang/barang/style.css') }}">
@endsection
@section('content')
        ...
        <div class="d-flex justify-content-start">
          <div class="dropdown">
            ...
            <div class="dropdown-menu" aria-labelledby="dropdownMenuIconButton1">
              ...
              <a href="#" class="dropdown-item filter-btn" data-filter="jenis_barang">Kategori Barang</a>
          ...
          <a href="{{ url('/barang/new') }}" class="btn btn-icons btn-inverse-primary btn-new ml-2">
      ...
      <div class="alert alert-primary d-flex justify-content-between align-items-center" role="alert">
        @if ($supply_system->status == false)
          <div class="text-instruction">
            <i class="mdi mdi-information-outline mr-2"></i> Aktifkan sistem stok dan pasok barang dengan klik tombol
            disamping
          </div>
          <a href="{{ url('/supply/system/active') }}" class="btn stok-btn">Aktifkan</a>
        @else
          <div class="text-instruction">
            <i class="mdi mdi-information-outline mr-2"></i> Nonaktifkan sistem stok dan pasok barang dengan klik tombol
            disamping
          </div>
              ...
              <table class="table table-custom">
                ...
                <tbody>
                  @foreach ($barangs as $barang)
                    <tr>
                      <td>
                        <span class="kd-barang-field">{{ $barang->kode_barang }}</span><br><br>
                        <span class="nama-barang-field">{{ $barang->nama_barang }}</span>
                      </td>
                      ...
                      @if ($supply_system->status == true)
                        <td><span class="ammount-box bg-secondary"><i
                              class="mdi mdi-cube-outline"></i></span>{{ $barang->stok }}</td>
                      @endif
                      <td><span class="ammount-box bg-green"><i class="mdi mdi-coin"></i></span>Rp.
                        {{ number_format($barang->harga, 2, ',', '.') }}</td>
                      @if ($supply_system->status == true)
                        <td>
                          @if ($barang->keterangan == 'Tersedia')
                            <span class="btn tersedia-span">{{ $barang->keterangan }}</span>
                          @else
                            <span class="btn habis-span">{{ $barang->keterangan }}</span>
                          @endif
                        </td>
                      @endif
                      <td></td>
                      <td>
                        <button type="button" class="btn btn-edit btn-icons btn-rounded btn-secondary"
                          data-toggle="modal" data-target="#editModal" data-edit="{{ $barang->id }}">
...
@section('script')
  ...
  <script src="{{ asset('js/manage_barang/barang/script.js') }}"></script>
  <script type="text/javascript">
    ...
    $(document).on('click', '.filter-btn', function(e) {
      ...
      $.ajax({
        method: "GET",
        url: "{{ url('/barang/filter') }}/" + data_filter,
    ...
    $(document).on('click', '.btn-edit', function() {
      ...
      $.ajax({
        method: "GET",
        url: "{{ url('/barang/edit') }}/" + data_edit,
        success: function(response) {
          $('input[name=id]').val(response.barang.id);
          $('input[name=kode_barang]').val(response.barang.kode_barang);
          $('input[name=nama_barang]').val(response.barang.nama_barang);
          $('input[name=stok]').val(response.barang.stok);
          $('input[name=harga]').val(response.barang.harga);
          var berat_barang = response.barang.berat_barang.split(" ");
          $('input[name=berat_barang]').val(berat_barang[0]);
          $('select[name=jenis_barang] option[value="' + response.barang.jenis_barang + '"]').prop('selected',
            true);
    ...
    $(document).on('click', '.btn-delete', function(e) {
      ...
      swal({
        ...
        .then((willDelete) => {
          if (willDelete) {
            window.open("{{ url('/barang/delete') }}/" + data_delete, "_self");
          }
        });
    });
  </script>
